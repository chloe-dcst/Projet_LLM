...
- Qualité rédactionnelle : rapport
...
- Ne conserver que les lignes contenant l'ensemble des informations demandées
...
Nous avons ensuite procédé à l'**analyse exploratoire des données** dans le but :
- D'identifier une problématique pertinente
...
**Fonctionnalités :**
- Ajouter une note en précisant le semestre, la compétence et la ressource concernée
- Modifier une note existante
- Supprimer une note en choisissant les 3 paramètres
- Supprimer la totalité des notes d'un ou des deux semestres

Pour cela, nous avons intégré des boutons et des listes déroulantes dans des UserForms.

**Partie Excel (visuel) :**
- Tableau de bord dynamique sur la page d'accueil
- Feuille de calcul pour chaque semestre
...
**Étapes du projet :**
1. Création d'un jeu de données inséré en SQL
2. Utilisation de Python pour importer et traiter des fichiers CSV fournis
3. Nettoyage et modification des fichiers pour compatibilité avec la base de données
4. Rédaction de 10 requêtes SQL pour extraire des informations pertinentes
5. Conception d'une interface utilisateur permettant :
   - De visualiser le contenu d'une table
...
- Un **échantillonnage aléatoire stratifié**

Nous disposions d'un fichier contenant les données de population de toutes les communes françaises. Nous avons extrait uniquement les données correspondant aux communes de la région étudiée, puis réalisé les estimations à partir de ces échantillons.
...
Après avoir récupéré les tables correspondant aux départements choisis, nous avons :
- Supprimé les colonnes inutiles pour alléger les jeux de données
- Préparé les données pour leur exploitation dans Power BI
- Conçu des graphiques interactifs
- Rédigé des paragraphes explicatifs pour interpréter les résultats
...
- Adéquation entre les graphiques et la problématique
...
Après le **nettoyage et le traitement des données**, nous avons réalisé une analyse statistique :
- Calcul de la proportion de volontaires accidentés selon le genre
- Analyse selon l'âge, avec des classes de 10 ans
- Test du khi-deux d'indépendance pour évaluer le lien entre le genre et la survenue d'un accident

**Problème rencontré :** faible représentativité des données (sur plus de 700 participants, seulement 33 avaient eu un accident).

Nous avons développé :
- Une interface utilisateur en Python (Tkinter) permettant d'ajouter et de modifier des données
- Un accès aux résultats de l'analyse statistique
- Une partie datavisualisation réalisée sur Power BI
- Un tutoriel utilisateur accessible depuis l'interface
...
- Création de supports de tutoriel
...
À partir de ces documents, nous avons :
- Construit le tableau des soldes intermédiaires de gestion
- Réalisé le bilan fonctionnel
- Créé les tableaux de calcul des indicateurs et des ratios financiers
...
- **PHP** : formulaires et gestion des utilisateurs
- **JavaScript/jQuery** : graphiques du tableau de bord

**Structure du site :**

**Partie utilisateur :**
- Accueil
- Recherche (formulaire de sélection d'ordinateurs)
- Commandes
- Avis
- Statistiques (graphiques jQuery)

**Partie Ordiginal Pro :**
- Gestion des tables `client`, `ordi` et `commande`
...
- Utilisation de PHP pour les formulaires
...
**Deuxième onglet :** carte choroplèthe, où l'on peut choisir la statistique représentée (nombre de logements, prix moyen, part de logement entier).

**Troisième onglet :** représentation des parts de type de logement par arrondissement sous forme de "small multiples".
...
- Fichier Excel source contenant la liste des candidats et les barèmes
- Création automatique de 3 feuilles : `RésultatsGardiens`, `RésultatsJoueurs07-08` et `RésultatsJoueurs09-10`
- Fiche individuelle par joueur accessible via pop-up

**2. Génération du classement :**
- Bouton "Générer les classements"
- Création de la feuille `Classementfinal` avec classement par groupe

**3. Tableau de bord :**
- Comparatif entre 2 joueurs avec graphique radar et graphiques barres
- Statistiques des clubs avec création de feuilles par club
- Deux tableaux : effectifs et moyennes des tests par catégorie
- Graphique circulaire modulable selon les filtres
...
Sur Excel, nous avons débuté en déterminant des catégories pour avoir un modèle par catégorie et tenter d'être le plus précis possible. Après avoir supprimé 10 % des biens les plus chers et les moins chers du fichier "train", nous avons séparé les maisons et les appartements, puis les appartements selon le nombre de pièces.

Nous avons subdivisé chaque catégorie en regroupant les logements par arrondissement et en calculant une moyenne des prix de vente. Nous avons testé les quatre modèles sur chaque catégorie pour déterminer le meilleur dans chaque cas. Notre conclusion a été que le modèle linéaire était le meilleur pour chaque catégorie.
...
**Démarche :**
- Récupération des données sur le site de l'OCDE
- Traitement sur Excel
- Étude descriptive des données pour analyser la tendance des importations australiennes
- Test de plusieurs modèles : quadratique, exponentiel, linéaire, logarithmique, puissance
- Choix du modèle **puissance** (meilleur R²) pour les prédictions
...
- Brève introduction de bienvenue
- Ma photo
- Les trois derniers projets réalisés

**Version mise à jour :**
- Style légèrement modifié pour mieux refléter ma personnalité
- Ajout des projets réalisés cette année
...
- VBA (automatisation Excel, interfaces utilisateur)
- SQL (requêtage, modélisation de bases de données)
- HTML/CSS/PHP/JavaScript (développement web)
...
- Présentation orale (français et anglais)
- Documentation utilisateur
- Argumentation de choix techniques
...
- **Adaptabilité** face à de nouveaux langages et outils
- **Communication** pour présenter mes travaux

---

*Ce portfolio de projets illustre ma progression et mon investissement tout au long de ma formation en BUT Science des Données. Chaque projet a contribué à développer mes compétences techniques et transversales, me préparant ainsi aux défis du monde professionnel.*
